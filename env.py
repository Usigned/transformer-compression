...
class QuantPruneEnv:
    ...
    def _estimate_strategy(self):
        return self.estimate_strategy(self.quant_strategy, self.prune_strategy, self.coeff_lat, self.coeff_e, self.a_bit)
...
class CombQuantPruneEnv:
    def __init__(self, model: CAFIA_Transformer, weight_path, trainloader, testloader, min_bit, max_bit, a_bit, max_heads, min_heads, head_dim, ori_acc, device, state_dim=7, float_bit=8, prune_only=False, lambda_acc=20, lambda_lat=1e-3, lambda_e=1e-3, lambda_mem=1e-5) -> None:
    ...
    def __init_resource_computation(self):
        self.coeff_lat = args.COEF_LAT
        self.coeff_e = args.COEF_E
        ori_resource_consumption = self.estimate_strategy(
            [self.float_bit]*len(self.quant_strategy),
            [self.max_heads]*len(self.prune_strategy),
            self.coeff_lat,
            self.coeff_e,
            self.float_bit
        )
        self.ori_lat, self.ori_e, self.ori_mem = ori_resource_consumption
    ...
    def reward(self):
        lat, e, mem = self._estimate_strategy()
        r_acc = (self.mem(self.strategy) -  self.ori_acc) * self.lambda_acc
        r_lat = (self.ori_lat - lat) * self.lambda_lat
        r_e = (self.ori_e - e) * self.lambda_e
        r_mem = (self.ori_mem - mem) * self.lambda_mem
        return r_acc + r_lat + r_e + r_mem
